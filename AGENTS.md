...
- Every domain skill keeps its standalone review format in `review-output.md`, reached from the Quick Reference, never in `SKILL.md`. These skills fire mostly on build tasks, where the review format is dead weight in context; the Quick Reference row is what makes it reachable on the review branch. `better-interface` owns the format whenever it orchestrates, and states that once, in its own principle 3 — `review-output.md` names the precedence in a line and defines nothing.
