...
- `stress-test` is user-invoked for the same reason as `variant`: it writes a throwaway harness, and an agent firing it after every component it touches would litter the repo with test pages.
...
| `stress-test` | Robustness testing of one component: the scenario axes and their applicability cues, the stress harness, the render-and-observe rule and the survived / broke report. Owns no domain rules and issues no verdict; each break names the domain skill that owns the fix. Isolates on purpose where `variant` insists on the real page |
