...
- A natively `disabled` control suppresses pointer events and leaves the tab order. A tooltip attached to it never opens for keyboard or touch users, and is unreliable for mouse users. Put the reason in persistent visible text beside the control, or switch to `aria-disabled="true"`, which keeps the control focusable and hoverable and can carry a tooltip.
