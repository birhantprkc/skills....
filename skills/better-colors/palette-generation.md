...
- **`prefers-color-scheme` alone** is correct when the product has no theme toggle. Nothing to persist, nothing to hydrate.
- **A `.dark` class** is required as soon as users can override the system setting. The media query then only sets the initial value.
